...
class SimplexNoise(seed: Int) {
    ...
    fun noise(x: Double, y: Double, z: Double, w: Double): Double {
        ...
        val i1: Int = if (rankx >= 3) 1 else 0
        val j1: Int = if (ranky >= 3) 1 else 0
        val k1: Int = if (rankz >= 3) 1 else 0
        val l1: Int = if (rankw >= 3) 1 else 0 // The integer offsets for the second simplex corner
        // Rank 2 denotes the second largest coordinate.
        val i2: Int = if (rankx >= 2) 1 else 0
        val j2: Int = if (ranky >= 2) 1 else 0
        val k2: Int = if (rankz >= 2) 1 else 0
        val l2: Int = if (rankw >= 2) 1 else 0 // The integer offsets for the third simplex corner
        // Rank 1 denotes the second smallest coordinate.
        val i3: Int = if (rankx >= 1) 1 else 0
        val j3: Int = if (ranky >= 1) 1 else 0
        val k3: Int = if (rankz >= 1) 1 else 0
        val l3: Int = if (rankw >= 1) 1 else 0 // The integer offsets for the fourth simplex corner
    ...
    companion object {
        ...
        private val grad3 = arrayOf(
            Tensor(1, 1, 0), Tensor(-1, 1, 0), Tensor(1, -1, 0), Tensor(-1, -1, 0),
            Tensor(1, 0, 1), Tensor(-1, 0, 1), Tensor(1, 0, -1), Tensor(-1, 0, -1),
            Tensor(0, 1, 1), Tensor(0, -1, 1), Tensor(0, 1, -1), Tensor(0, -1, -1)
        )
        private val grad4 = arrayOf(
            Tensor(0, 1, 1, 1), Tensor(0, 1, 1, -1), Tensor(0, 1, -1, 1), Tensor(0, 1, -1, -1),
            Tensor(0, -1, 1, 1), Tensor(0, -1, 1, -1), Tensor(0, -1, -1, 1), Tensor(0, -1, -1, -1),
            Tensor(1, 0, 1, 1), Tensor(1, 0, 1, -1), Tensor(1, 0, -1, 1), Tensor(1, 0, -1, -1),
            Tensor(-1, 0, 1, 1), Tensor(-1, 0, 1, -1), Tensor(-1, 0, -1, 1), Tensor(-1, 0, -1, -1),
            Tensor(1, 1, 0, 1), Tensor(1, 1, 0, -1), Tensor(1, -1, 0, 1), Tensor(1, -1, 0, -1),
            Tensor(-1, 1, 0, 1), Tensor(-1, 1, 0, -1), Tensor(-1, -1, 0, 1), Tensor(-1, -1, 0, -1),
            Tensor(1, 1, 1, 0), Tensor(1, 1, -1, 0), Tensor(1, -1, 1, 0), Tensor(1, -1, -1, 0),
            Tensor(-1, 1, 1, 0), Tensor(-1, 1, -1, 0), Tensor(-1, -1, 1, 0), Tensor(-1, -1, -1, 0)
        )
        ...
        private fun dot(g: Tensor, x: Double, y: Double): Double {
            return g.x * x + g.y * y
        }

        private fun dot(g: Tensor, x: Double, y: Double, z: Double): Double {
            return g.x * x + g.y * y + g.z * z
        }

        private fun dot(g: Tensor, x: Double, y: Double, z: Double, w: Double): Double {
